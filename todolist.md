...
### Phase 2: 图片生成逻辑拆分 (2-3天) ✅ 已完成

#### 2.1 创建图片生成 Hooks 目录
- [x] ✅ 创建 `hooks/canvas/` 目录
- [x] ✅ 设计图片生成类型定义 (`types/image-generation.ts`)

#### 2.2 拆分文生图逻辑
- [x] ✅ 创建 `useTextToImage.ts` Hook
- [x] ✅ 迁移 `handleTextToImage()` 函数
- [x] ✅ 处理依赖: `addElement`, `updateElement`, `setEdges`

#### 2.3 拆分图生图逻辑
- [x] ✅ 创建 `useImageToImage.ts` Hook
- [x] ✅ 迁移 `handleImageToImage()` 函数
- [x] ✅ 保持 `handleImagePromptInputChange()` 和 `handleConfirmImagePrompt()` 在 Canvas 中

#### 2.4 工具函数提取
- [x] ✅ `getImageSizeFromRatio()` - 图片尺寸计算
- [x] ✅ `getVideoSizeFromRatio()` - 视频尺寸计算
- [x] ✅ `getTargetPositionRightOf()` - 位置计算
- [x] ✅ `ImageAspectRatio` 和 `VideoAspectRatio` 类型定义

#### 2.5 整合和测试
- [x] ✅ 在 Canvas.tsx 中引入新 Hooks
- [x] ✅ 删除原有的图片生成函数 (~155行)
- [x] ✅ TypeScript 编译通过
- [x] ✅ 构建成功

**实际代码减少**: 155 行 → Canvas.tsx 从 1312 行减少到 1157 行 (-11.8%)
...
| Phase 2 ✅ | 1157 | -155 | 23.6% |
| Phase 3 | ~957 | -200 | 36.8% |
| Phase 4 | ~657 | -300 | 56.6% |
| Phase 5 | ~500 | -157 | 67.0% |

### 本周目标 (Week 1)
- [x] ✅ 完成 Phase 1: 连线菜单系统拆分 (2025-11-18)
- [x] ✅ 完成 Phase 2: 图片生成逻辑拆分 (2025-11-18)
